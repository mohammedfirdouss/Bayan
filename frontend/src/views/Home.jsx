    ...
    <div style={{ paddingTop: 64 }}>
      ...
      <section style={s.hero}>
        ...
        <div style={s.heroContent}>
          ...
          <h1 style={s.heroTitle}>
            Prepare Your Khutbah<br />with <em style={{ color: 'var(--gold)', fontStyle: 'italic' }}>Certainty</em>
          </h1>
          <p style={s.heroSub}>
            The AI-powered preparation companion for Khateebs.<br />
            Verify citations. Discover ayat by meaning. Structure your message.<br />
            Step onto the mimbar with confidence.
          </p>
          <p style={s.heroAyah}>"...a clarification of all things" — Quran 16:89</p>
      ...
      <section ref={addRef} style={{ ...s.section, ...s.sectionCream }} className="fade-section">
        ...
        <p style={s.sectionBody}>
          Hours spent searching Quran apps, verifying authenticity across multiple websites,
          cross-referencing scholarly opinions — all before you've shaped your message.
        </p>
        <div style={s.painGrid}>
          {[
            { icon: '📖', title: 'Scattered Sources', body: 'Jumping between Quran apps, sunnah.com, Dorar.net, and physical books just to compile references for a single Khutbah.' },
            { icon: '⚖️', title: 'Authentication Anxiety', body: "Is that hadith sahih or da'if? Misquoting from the mimbar erodes trust with the congregation. Verification takes time." },
            { icon: '⏱️', title: 'Lost Focus', body: 'The best part of preparation — crafting the spiritual message — gets crowded out by citation logistics.' },
          ].map((p, i) => (
            <div key={i} ref={addRef} style={s.painCard} className="fade-section pain-card">
      ...
      <section style={{ ...s.section, background: 'var(--parchment)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={s.whatInner}>
          <div ref={addRef} style={s.whatText} className="fade-section">
            ...
            <h2 style={{ ...s.sectionTitle, textAlign: 'left' }}>Your Scholarly<br />Research Assistant</h2>
            <p style={{ ...s.sectionBody, margin: 0 }}>
              Bayan is an AI-powered preparation tool for Khateebs and scholars.
              Enter your Khutbah topic or a verse you half-remember — Bayan searches the
              full Quran corpus by meaning, verifies citations, and structures an outline.
            </p>
            <br />
            <p style={{ ...s.sectionBody, margin: 0 }}>
              It does not write your Khutbah. Your voice, your reasoning, your pastoral
              wisdom remain entirely yours. Bayan handles the research.
            </p>
          ...
          <div ref={addRef} style={s.demoBox} className="fade-section">
            <div style={s.demoHeader}>
              <div style={s.demoDot} /><div style={{ ...s.demoDot, opacity: 0.5 }} /><div style={{ ...s.demoDot, opacity: 0.3 }} />
              <span style={s.demoLabel}>Bayan · Live Preview</span>
            </div>
            <div style={s.demoInput}>
              <div style={s.demoInputLabel}>Khutbah Topic</div>
              <div style={s.demoInputText}>"Patience during hardship and trusting in Allah's plan"</div>
            </div>
            {[
              {
                ...
                trans: '"And We will surely test you with something of fear and hunger and a loss of wealth and lives and fruits..."',
              },
              {
          ...
          {[
            {
              ...
              icon: <svg viewBox="0 0 24 24" fill="none" stroke="var(--gold)" strokeWidth="1.5" width="28" height="28"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35" strokeLinecap="round"/></svg>,
              action: 'search',
            },
            {
              n: '2', title: 'Verify',
              ...
              icon: <svg viewBox="0 0 24 24" fill="none" stroke="var(--gold)" strokeWidth="1.5" width="28" height="28"><path d="M9 12l2 2 4-4"/><circle cx="12" cy="12" r="10"/></svg>,
              action: 'verify',
            },
            {
              n: '3', title: 'Outline',
              ...
              icon: <svg viewBox="0 0 24 24" fill="none" stroke="var(--gold)" strokeWidth="1.5" width="28" height="28"><path d="M4 6h16M4 12h16M4 18h10" strokeLinecap="round"/></svg>,
              action: 'outline',
            },
          ].map((f, i) => (
            <div
      ...
      <section style={{ ...s.section, background: 'var(--emerald-deep)', position: 'relative', overflow: 'hidden', textAlign: 'center' }}>
        ...
        <div style={s.notGrid}>
          {[
            { yes: true,  text: <><strong>Surfaces verified verses</strong> from the Quran corpus with exact references</> },
            { yes: false, text: <><strong>Does not generate</strong> Khutbah content — your words remain your own</> },
            { yes: true,  text: <><strong>Returns tafsir excerpts</strong> attributed to named scholars (Ibn Kathir, al-Sa'di)</> },
            { yes: false, text: <><strong>Does not offer opinions</strong> — only corpus-retrieved text is shown</> },
          ].map((item, i) => (
            <div key={i} ref={addRef} style={s.notItem} className="fade-section">
              <div style={{ ...s.notCheck, ...(item.yes ? s.notYes : s.notNo) }}>{item.yes ? '✓' : '✗'}</div>
              <p style={{ fontSize: 15, color: 'rgba(247,242,232,0.85)', lineHeight: 1.5 }}>{item.text}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section ref={addRef} style={{ ...s.section, background: 'var(--parchment)', textAlign: 'center' }} className="fade-section">
        ...
        <div style={s.roadmapTrack}>
          {[
            { num: '١', title: 'Khateeb Preparation Tool', body: 'Semantic Quran search, citation verification, Khutbah outline generation with Ibn Kathir tafsir.', tag: 'now', tagLabel: 'Building Now' },
            { num: '٢', title: 'Hadith Search & Grading', body: 'Search the six major hadith collections by meaning, with authenticity grading from classical scholars.', tag: 'next', tagLabel: 'Next' },
            { num: '٣', title: 'Post-Khutbah Recap', body: "AI-generated summaries with every ayah cited, sent to congregants after Jumu'ah.", tag: 'next', tagLabel: 'Next' },
            { num: '٤', title: 'Halaqah Live Display', body: 'Real-time projection of ayat as the teacher speaks during study circles.', tag: 'later', tagLabel: 'Later' },
          ].map((item, i) => (
            <div key={i} ref={addRef} style={{ ...s.roadmapItem, ...(i === 0 ? s.roadmapItemActive : {}) }} className="fade-section">
              <div style={{ ...s.roadmapDot, ...(i === 0 ? s.roadmapDotActive : {}) }}>{item.num}</div>
              <div style={{ textAlign: 'left' }}>
                <h3 style={s.roadmapTitle}>{item.title}</h3>
                <p style={s.roadmapBody}>{item.body}</p>
                <span style={{ ...s.roadmapTag, ...(item.tag === 'now' ? s.tagNow : item.tag === 'next' ? s.tagNext : s.tagLater) }}>
                  {item.tagLabel}
                </span>
              </div>
            </div>
      ...
      <footer style={s.footer}>
        ...
        <div style={s.footerLinks}>
          {['Search', 'Verify', 'Outline'].map(v => (
            <button key={v} onClick={() => onNavigate(v.toLowerCase())} style={s.footerLink}>{v}</button>
          ))}
        </div>
...
const s = {
  ...
  hero: {
    ...
    background: 'linear-gradient(175deg, var(--emerald-deep) 0%, #0A3D2A 45%, #072E20 100%)',
    overflow: 'hidden',
  },
  heroGlow: {
    position: 'absolute',
  ...
  demoDot: { width: 10, height: 10, borderRadius: '50%', background: 'var(--gold)' },
  demoLabel: {
    fontFamily: "'DM Sans', sans-serif",
    fontSize: 11,
    color: 'var(--gold-glow)',
  ...
  roadmapBody: { fontSize: 15, color: 'var(--ink-muted)', lineHeight: 1.6, marginBottom: 8 },
  roadmapTag: {
  ...
  tagNext: { background: 'rgba(26,138,101,0.1)',   color: 'var(--emerald-light)' },
  ...
  footerLinks: { display: 'flex', gap: 24, justifyContent: 'center', marginBottom: 20 },
  footerLink: {
    fontSize: 13,
