...
export default function VerseCard({
  ...
  const tafsirText =
    tafsir?.text_plain || tafsir?.text || null
      ...
      {!simplified && tafsirText && (
        <div style={cardStyles.tafsirSection}>
          ...
          >
            ...
            <span style={{ transition: 'transform 0.2s', display: 'inline-block', transform: tafsirOpen ? 'rotate(90deg)' : 'rotate(0deg)' }}>→</span>
          </button>
          {tafsirOpen && (
...
function getMatchBadgeStyle(t) {
  ...
  if (t === 'verified_exact') return { ...base, background: 'var(--emerald-deep)', color: 'var(--gold-glow)' }
  if (t === 'verified_paraphrase') return { ...base, background: '#B8860B', color: '#FFF8E1' }
  if (t === 'not_found') return { ...base, background: '#7B1818', color: '#FFE4E4' }
  return { ...base, background: 'var(--parchment-dark)', color: 'var(--ink-soft)' }
}
